...
const subtract = (a, b) => [a, b].map(n => [...n].reverse()).reduce((a, b) => a.reduce((r, d, i) => {
  ...
  describe("Freelancer", () => {
    ...
    it('Client Funded and Approved', async () => {
      ...
      expect(await freelancerApp.connect(client).fundWork("test task", taskvalue, freelancer.address, { value: taskvalue}))
    ...
    it('Client calling freelancerVote', async () =>  {
      ...
      freelancerApp.connect(client).fundWork("test task 01", 1, freelancer1.address, { value: 1})
      freelancerApp.connect(client).fundWork("test task 02", 1, freelancer2.address, { value: 1})

      const clientFundedTasks = await freelancerApp.connect(client).getTaskForClient(client.address)

      expect(clientFundedTasks[0].toString()).to.equal('0')
      expect(clientFundedTasks[1].toString()).to.equal('1')
      ...
      expect(freelancer2Task[0].toString()).to.equal('1')
    ...
    it('Client calling freelancerVote', async () =>  {
      ...
      freelancerApp.connect(client1).fundWork("test task 01", 1, freelancer.address, { value: 1})
      freelancerApp.connect(client2).fundWork("test task 02", 1, freelancer.address, { value: 1})

      const freelancerTasks = await freelancerApp.connect(freelancer).getTaskForFreelancer(freelancer.address)

      expect(freelancerTasks[0].toString()).to.equal('0')
      expect(freelancerTasks[1].toString()).to.equal('1')
    });
