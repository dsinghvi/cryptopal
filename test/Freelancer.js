...
describe('Freelancer', () => {
  ...
  it('Client Funded and Approved', async () => {
    ...
    expect(
      freelancerApp
        .connect(client)
        .fundWork(1, 'test task', 1, freelancer.address, {
          value: 12,
        }),
  ...
  it('Client Funded and Approved', async () => {
    ...
    expect(
      await freelancerApp
        .connect(client)
        .fundWork(1, 'test task', taskvalue, freelancer.address, {
          value: taskvalue,
        }),
    ).to.emit(freelancerApp, 'workFunded');

    expect(
      await freelancerApp.connect(client).clientVote(1, 0),
    ).to.emit(freelancerApp, 'transferFunds');
    const balanceAfter = await prov.getBalance(freelancer.address);

    expect(
      subtract(balanceAfter.toString(), balanceBefore.toString()),
    ).to.equal(taskvalue.toString());
  });
  ...
  it('Client Funded and Freelancer Declined Payment', async () => {
    ...
    expect(
      await freelancerApp.connect(freelancer).freelancerVote(1, 1),
    ).to.emit(freelancerApp, 'transferFunds');
  });
  ...
  it('Client calling freelancerVote', async () => {
    ...
    freelancerApp
      .connect(client)
      .fundWork(1, 'test task', 1, freelancer.address, { value: 1 });
    expect(
      freelancerApp.connect(freelancer).clientVote(0, 1),
  ...
  it('Client calling freelancerVote', async () => {
    ...
    freelancerApp
      ...
      .fundWork(1, 'test task', 1, freelancer.address, { value: 1 });
    expect(
      freelancerApp.connect(client).freelancerVote(0, 1),
    ).to.be.revertedWith('Only freelancer can call this flow');
  });

  // 6. Client with multiple tasks
  it('Client calling freelancerVote', async () => {
    const [client, freelancer1, freelancer2] =
      await ethers.getSigners();
    freelancerApp
      .connect(client)
      .fundWork(1, 'test task 01', 1, freelancer1.address, {
        value: 1,
      });
    freelancerApp
      .connect(client)
      .fundWork(23, 'test task 02', 1, freelancer2.address, {
        value: 1,
      });

    const clientFundedTasks = await freelancerApp
      .connect(client)
      .getTaskForClient(client.address);
    ...
    const freelancer2Task = await freelancerApp
      .connect(client)
      .getTaskForFreelancer(freelancer2.address);
  ...
  it('Client calling freelancerVote', async () => {
    ...
    freelancerApp
      .connect(client1)
      .fundWork(1, 'test task 01', 1, freelancer.address, {
        value: 1,
      });
    freelancerApp
      .connect(client2)
      .fundWork(12, 'test task 02', 1, freelancer.address, {
        value: 1,
      });

    const freelancerTasks = await freelancerApp
      .connect(freelancer)
      .getTaskForFreelancer(freelancer.address);
